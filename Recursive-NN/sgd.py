...
class SGD:
    ...
    def __init__(self,model,alpha=1e-2,minibatch=30,
                 optimizer='sgd', clipmin=-1, clipmax=1):
        self.model = model
    ...
    def run(self,trees):
        ...
        for i in xrange(0,m-self.minibatch+1,self.minibatch):
            ...
            if self.optimizer == 'sgd':
                ...
                self.gradt = [gt+g**2 for gt,g in zip(self.gradt,grad)]
                # update = grad.*trace.^(-1/2)
                #update =  [g*(1./np.sqrt(gt))
                        #for gt,g in zip(self.gradt[1:],grad[1:])]
                update =  [g*(1./np.sqrt(gt)) for gt,g in zip(self.gradt,grad)]
            ...
            #Clipping Gradient - (Our modification - Date 5th april)
            #_update = []
            #for g in update:
            #    g[g < self.clipmin] = self.clipmin
            #    g[g > self.clipmax] = self.clipmax
            #    _update.append(g)
