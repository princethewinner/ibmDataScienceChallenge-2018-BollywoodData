...
class RNN2:
    ...
    def initParams(self,L):
        ...
        self.stack = [self.W1, self.b1, self.W2, self.b2, self.Ws, self.bs]
    ...
    def costAndGrad(self,mbdata,test=False):
        ...
        self.W1, self.b1, self.W2, self.b2, self.Ws, self.bs = self.stack
        # Zero gradients
        self.dW1[:] = 0
        self.db1[:] = 0
        ...
        actss = []
        for tree in mbdata:
            c,tot, _actss = self.forwardProp(tree.root,correct,guess)
            cost += c
            total += tot
            actss.append(_actss)

        if test:
            return (1./len(mbdata))*cost,correct, guess, total, actss
        ...
        return scale*cost,[scale*(self.dW1 + self.rho*self.W1),scale*self.db1,
    ...
    def forwardProp(self,node, correct=[], guess=[]):
        ...
        # Recursion
        if not node.isLeaf:
            left_cost, left_total, _ = self.forwardProp(node.left, correct, guess)
            right_cost, right_total, _ = self.forwardProp(node.right, correct, guess)
        ...
        for p in range(node.probs.shape[0]):
            try:
                _e = np.exp(node.probs[p])
            except:
                print(node.probs[p])
                _e = 0
            node.probs[p] = _e
    ...
    def updateParams(self,scale,update,log=False):
        ...
        if log:
            ...
            for P,dP in zip(self.stack,update):
                pRMS = np.sqrt(np.mean(P**2))
                dpRMS = np.sqrt(np.mean((scale*dP)**2))
                print "weight rms=%f -- update rms=%f"%(pRMS,dpRMS)

        #self.stack[1:] = [P+scale*dP for P,dP in zip(self.stack[1:],update[1:])]
        self.stack = [P+scale*dP for P,dP in zip(self.stack,update)]
    ...
    def check_grad(self,data,epsilon=1e-6):
        ...
        for W,dW in zip(self.stack,grad):
